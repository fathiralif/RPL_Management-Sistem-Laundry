...
<template id="itemTemplate">
    <div class="item-row" style="background:var(--gray-50);border:1.5px solid var(--gray-200);border-radius:12px;padding:14px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px;">
            ...
            <div>
                <label style="font-size:11px;font-weight:600;color:var(--gray-600);margin-bottom:4px;display:block;">Berat (kg) *</label>
    ...
    document.querySelectorAll('.item-row').forEach(row => {
        ...
        if (type && w > 0) {
            ...
            summaryList.innerHTML += `<div style="display:flex;justify-content:space-between;font-size:12px;padding:4px 0;border-bottom:1px dashed var(--gray-100);">
                <span style="color:var(--gray-700);">${icon} ${type.charAt(0).toUpperCase()+type.slice(1)}</span>
                <span style="font-weight:600;">${w} kg</span>
            </div>`;
        }
    });

    document.getElementById('weightDisplay').textContent = totalW.toFixed(1) + ' kg';
    document.getElementById('totalWeightText').textContent = totalW.toFixed(1) + ' kg';
